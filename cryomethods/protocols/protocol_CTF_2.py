...
from cryomethods.functions import NumpyImgHandler, polyfit2d
...
class Protdctf2(ProtocolBase):
    ...
    def predict_CTF(self, images_path, window_size):
        ...
        data_loader = DataLoader(trainset, batch_size=self.batch_size.get(), shuffle=False, num_workers=0,
                                 pin_memory=True)
    ...
    with torch.no_grad():
        for data in data_loader:
            ...
            for batch_id in range(0, size):
                ...
                NumpyImgHandler.saveMrc(np.float32(image[batch_id, :, :, :]), filename)
...
class LoaderPredict(Dataset):
    """
    Class to load the dataset for predict and do some preprocessing.
    """
    def __init__(self, datafiles, normalization_path, window_size, step_size, sampling_rate, sampling):
        ...
        # Transformaciones adicionales, como convertir las imágenes a tensores y normalizar
    ...
    def __getitem__(self, index):
        ...
        for tensor in img:
            return {'image': tensor, 'name': img_path}
    ...
    def open_image(self, filename):
        ...
        # Resize image if necesary to adjust downsampling
        new_size = (int(img.shape[1] * self.sampling_rate / self.sampling), int(img.shape[0] * self.sampling_rate / self.sampling))
        PIL_image = Image.fromarray(img)
        resized_image = PIL_image.resize(new_size, resample=Image.BICUBIC)
        img = np.asarray(resized_image)

        #tensor_img = to_tensor(img)
        #resized_tensor_img = F.interpolate(tensor_img.unsqueeze(0), size=new_size, mode='bicubic', align_corners=False)
        #resized_img = to_pil_image(resized_tensor_img.squeeze())
        ...
        img_transformed = img_transformed.numpy() - polyfit2d(x, y, img_transformed.numpy(), kx=2, ky=2, order=2)
        #psd = calcAvgPsd(img, windows_size=self._window_size, step_size=self.step_size)
        #return torch.from_numpy(np.float32(psd))
        #return torch.from_numpy(np.float32(img))
        return torch.from_numpy(np.float32(img_transformed))
...
def custom_transform(batch):
    ...
    for image in batch:
        ...
        # Dividir la imagen en subimágenes de tamaño 256x256 con un paso de 128
        for i in range(0, image.shape[0] - 256, 128):
            for j in range(0, image.shape[1] - 256, 128):
                subimage = image[i:i + 256, j:j + 256]
                # Realizar la transformada de Fourier y obtener el valor absoluto
                subimage_fft = torch.fft.fftn(subimage)
                subimage_fft = torch.fft.fftshift(subimage_fft)
                subimage_fft = torch.abs(subimage_fft)

                # Aplicar el logaritmo
                subimage_fft = torch.log(subimage_fft)
                q_80 = torch.quantile(subimage_fft, 0.98)
                q_20 = torch.quantile(subimage_fft, 0.1)
                subimage_fft[subimage_fft >= q_80] = q_80
                subimage_fft[subimage_fft < q_20] = q_20
                subimages.append(subimage_fft)

        # Calcular el promedio de las subimágenes en GPU
        if len(subimages) > 0:
            avg_subimage = torch.mean(torch.stack(subimages, dim=0), dim=0)
            x = np.linspace(-1, 1, 256)
            y = np.linspace(-1, 1, 256)
            avg_subimage = avg_subimage.cpu().numpy() - polyfit2d(x, y, avg_subimage.cpu().numpy(), kx=2, ky=2, order=2)
            avg_subimage = torch.from_numpy(avg_subimage).float()
            q_80 = torch.quantile(avg_subimage, 0.98)
            q_20 = torch.quantile(avg_subimage, 0.02)
            avg_subimage[avg_subimage >= q_80] = q_80
            avg_subimage[avg_subimage < q_20] = q_20
        else:
            avg_subimage = torch.zeros_like(image)

        transformed_batch.append(avg_subimage)

    transformed_batch = torch.stack(transformed_batch, dim=0)
