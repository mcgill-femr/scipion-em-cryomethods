...
from pwem.objects import Angles
...
if not getattr(xmippLib, "GHOST_ACTIVATED", False):
    ...
    ACQUISITION_DICT = OrderedDict([
           ("_amplitudeContrast", MDL_CTF_Q0),
           ("_sphericalAberration", MDL_CTF_CS),
           ("_voltage", MDL_CTF_VOLTAGE)
           ])
    ...
    CTF_DICT = OrderedDict([
           ("_defocusU", MDL_CTF_DEFOCUSU),
           ("_defocusV", MDL_CTF_DEFOCUSV),
           ("_defocusAngle", MDL_CTF_DEFOCUS_ANGLE),
           ("_resolution", MDL_CTF_CRIT_MAXFREQ),
           ("_fitQuality", MDL_CTF_CRIT_FITTINGSCORE)
           ])
    ...
    CTF_PSD_DICT = OrderedDict([
           ("_psdFile", MDL_PSD),
           ("_xmipp_enhanced_psd", MDL_PSD_ENHANCED),
           ("_xmipp_ctfmodel_quadrant", MDL_IMAGE1),
           ("_xmipp_ctfmodel_halfplane", MDL_IMAGE1)
           ])
    ...
    ANGLES_DICT = OrderedDict([
           ("_angleY",  MDL_ANGLE_Y),
           ("_angleY2",  MDL_ANGLE_Y2),
           ("_angleTilt",  MDL_ANGLE_TILT)
           ])

    ALIGNMENT_DICT = OrderedDict([
           ("_xmipp_shiftX",  MDL_SHIFT_X),
           ("_xmipp_shiftY",  MDL_SHIFT_Y),
           ("_xmipp_shiftZ",  MDL_SHIFT_Z),
           ("_xmipp_flip",  MDL_FLIP),
           ("_xmipp_anglePsi",  MDL_ANGLE_PSI),
           ("_xmipp_angleRot",  MDL_ANGLE_ROT),
           ("_xmipp_angleTilt",  MDL_ANGLE_TILT),
           ])
...
def objectToRow(obj, row, attrDict, extraLabels=[]):
    ...
    if obj.isEnabled():
        ...
        enabled = -1
    row.setValue( MDL_ENABLED, enabled)
    ...
    for label in extraLabels:
        attrName = '_xmipp_%s' %  label2Str(label)
...
def rowToObject(row, obj, attrDict, extraLabels=[]):
    ...
    Params:
        ...
        extraLabels: a list with extra labels that could be included
            as _xmipp_labelName
    """
    obj.setEnabled(row.getValue( MDL_ENABLED, 1) > 0)
...
def setXmippAttribute(obj, label, value):
    ...
    setattr(obj, prefixAttribute( label2Str(label)), value)
...
def xmippToLocation(xmippFilename):
    ...
    if '@' in xmippFilename:
        return  FileName(xmippFilename).decompose()
    else:
        return NO_INDEX, str(xmippFilename)


def setObjId(obj, mdRow, label= MDL_ITEM_ID):
...
def setRowId(mdRow, obj, label= MDL_ITEM_ID):
...
def rowToImage(imgRow, imgLabel, imgClass, **kwargs):
    ...
    if imgRow.containsLabel( MDL_REF):
        img.setClassId(imgRow.getValue( MDL_REF))
    elif imgRow.containsLabel( MDL_REF3D):
        img.setClassId(imgRow.getValue( MDL_REF3D))
...
def coordinateToRow(coord, coordRow, copyId=True):
    ...
    if coord.getMicId():
        coordRow.setValue( MDL_MICROGRAPH, str(coord.getMicId()))
...
def rowToCoordinate(coordRow):
    ...
    if _containsAll(coordRow, COOR_DICT):
        ...
        try:
            coord.setMicId(int(coordRow.getValue( MDL_MICROGRAPH_ID)))
        except Exception:
            pass
    else:
        coord = None
...
def _rowToParticle(partRow, particleClass, **kwargs):
    ...
    img = rowToImage(partRow,  MDL_IMAGE, particleClass, **kwargs)
    img.setCoordinate(rowToCoordinate(partRow))
    # copy micId if available
    # if not copy micrograph name if available
    try:
        if partRow.hasLabel( MDL_MICROGRAPH_ID):
            img.setMicId(partRow.getValue( MDL_MICROGRAPH_ID))
#        elif partRow.hasLabel( MDL_MICROGRAPH):
#            micName = partRow.getValue( MDL_MICROGRAPH)
...
def particleToRow(part, partRow, **kwargs):
    ...
    imageToRow(part, partRow,  MDL_IMAGE, **kwargs)
    coord = part.getCoordinate()
    if coord is not None:
        coordinateToRow(coord, partRow, copyId=False)
    if part.hasMicId():
        partRow.setValue(MDL_MICROGRAPH_ID, int(part.getMicId()))
        partRow.setValue(MDL_MICROGRAPH, str(part.getMicId()))
...
def rowToClass(classRow, classItem):
    ...
    a row of a metadata
    """
    setObjId(classItem, classRow, label= MDL_REF)

    if classRow.containsLabel( MDL_IMAGE):
        index, filename = xmippToLocation(classRow.getValue( MDL_IMAGE))
...
def class2DToRow(class2D, classRow):
    ...
    if class2D.hasRepresentative():
        ...
        fn = locationToXmipp(index, filename)
        classRow.setValue( MDL_IMAGE, fn)
    n = int(len(class2D))
    classRow.setValue( MDL_CLASS_COUNT, n)
    classRow.setValue( MDL_REF, int(class2D.getObjId()))
    classRow.setValue( MDL_ITEM_ID, int(class2D.getObjId()))
...
def ctfModelToRow(ctfModel, ctfRow):
    ...
    if ctfModel.hasResolution():
        ...
        if ctfModel.hasPhaseShift():
            ctfRow.setValue( MDL_CTF_PHASE_SHIFT,
...
def rowToCtfModel(ctfRow):
    ...
    if _containsAll(ctfRow, CTF_DICT_NORESOLUTION):
        ...
        if ctfRow.containsLabel( label2Str( MDL_CTF_CRIT_MAXFREQ)):
...
def writeCoordsConfig(configFn, boxSize, state):
    ...
    md.setValue( MDL_PICKING_STATE, state, objId)
...
def readSetOfCoordinates(outputDir, micSet, coordSet, readDiscarded=False):
    ...
    if exists(configfile):
        md =  MetaData('properties@' + join(outputDir, 'config.xmd'))
        boxSize = md.getValue( MDL_PICKING_PARTICLE_SIZE,
                              md.firstObject())
...
def readSetOfImages(filename, imgSet, rowToFunc, **kwargs):
    ...
    if 'alignType' not in kwargs:
        ...
        if _containsAny(imgRow, ALIGNMENT_DICT):
            if imgRow.containsLabel( MDL_ANGLE_TILT):
...
#    micMd.removeDisabled()
    ...
    for objId in micMd:
        ...
        angles.setObjId(micMd.getValue( MDL_ITEM_ID, objId))
...
def writeSetOfCTFs(ctfSet, mdCTF):
    ...
    for ctfModel in ctfSet:
        objId = md.addObject()
        ctfRow = XmippMdRow()
        ctfRow.setValue( MDL_MICROGRAPH, ctfModel.getMicFile())
        if ctfModel.getPsdFile():
            ctfRow.setValue( MDL_PSD, ctfModel.getPsdFile())
        ctfModelToRow(ctfModel, ctfRow)
        ctfRow.writeToMd(md, objId)
...
def writeSetOfDefocusGroups(defocusGroupSet, fnDefocusGroup):  # also metadata
    ...
    for defocusGroup in defocusGroupSet:
        ...
        defocusGroupRow.setValue( MDL_CTF_GROUP, defocusGroup.getObjId())
        defocusGroupRow.setValue( MDL_MIN, defocusGroup.getDefocusMin())
        defocusGroupRow.setValue( MDL_MAX, defocusGroup.getDefocusMax())
        defocusGroupRow.setValue( MDL_AVG, defocusGroup.getDefocusAvg())
...
def writeSetOfClasses2D(classes2DSet, filename,
    ...
    for class2D in classes2DSet:
        ...
        if writeParticles:
            ...
            imagesMd.write(imagesFn,  MD_APPEND)

    # Empty write to ensure the classes is the first block
    classMd.write(classFn,  MD_APPEND)
...
def writeSetOfMicrographsPairs(uSet, tSet, filename):
    ...
    for micU, micT in izip(uSet, tSet):
        objId = md.addObject()
        pairRow = XmippMdRow()
        pairRow.setValue( MDL_ITEM_ID, int(micU.getObjId()))
        pairRow.setValue( MDL_MICROGRAPH, micU.getFileName())
        pairRow.setValue( MDL_MICROGRAPH_TILTED, micT.getFileName())
...
def __readSetOfClasses(classBaseSet, readSetFunc,
    ...
    """
    blocks =  getBlocksInMetaDataFile(filename)

    classesMd =  MetaData('%s@%s' % (classesBlock, filename))
...
def rowToAlignment(alignmentRow, alignType):
    ...
    if _containsAny(alignmentRow, ALIGNMENT_DICT):
        ...
        flip = alignmentRow.getValue( MDL_FLIP)

        shifts[0] = alignmentRow.getValue( MDL_SHIFT_X, 0.)
        shifts[1] = alignmentRow.getValue( MDL_SHIFT_Y, 0.)
        if not is2D:
            angles[0] = alignmentRow.getValue( MDL_ANGLE_ROT, 0.)
            angles[1] = alignmentRow.getValue( MDL_ANGLE_TILT, 0.)
            shifts[2] = alignmentRow.getValue( MDL_SHIFT_Z, 0.)
            angles[2] = alignmentRow.getValue( MDL_ANGLE_PSI, 0.)
            if flip:
                angles[1] = angles[1]+180  # tilt + 180
                angles[2] = - angles[2]    # - psi, COSS: this is mirroring X
                shifts[0] = -shifts[0]     # -x
        else:
            psi = alignmentRow.getValue( MDL_ANGLE_PSI, 0.)
            rot = alignmentRow.getValue( MDL_ANGLE_ROT, 0.)
            if rot != 0. and psi != 0:
                print("HORROR rot and psi are different from zero in 2D case")
            angles[0] = \
                alignmentRow.getValue( MDL_ANGLE_PSI, 0.)\
                + alignmentRow.getValue( MDL_ANGLE_ROT, 0.)
...
def alignmentToRow(alignment, alignmentRow, alignType):
    ...
    alignmentRow.setValue( MDL_SHIFT_X, shifts[0])
    alignmentRow.setValue( MDL_SHIFT_Y, shifts[1])

    if is2D:
        angle = angles[0] + angles[2]
        alignmentRow.setValue( MDL_ANGLE_PSI,  angle)
    else:
        # if alignType == ALIGN_3D:
        alignmentRow.setValue( MDL_SHIFT_Z, shifts[2])
        alignmentRow.setValue( MDL_ANGLE_ROT,  angles[0])
        alignmentRow.setValue( MDL_ANGLE_TILT, angles[1])
        alignmentRow.setValue( MDL_ANGLE_PSI,  angles[2])
    alignmentRow.setValue( MDL_FLIP, flip)
...
def writeShiftsMovieAlignment(movie, xmdFn, s0, sN):
    ...
    if s0 < a0:
        for i in range(s0, a0):
            ...
            globalShiftsMD.setValue( MDL_IMAGE, imgFn, objId)
            globalShiftsMD.setValue( MDL_SHIFT_X, 0.0, objId)
            globalShiftsMD.setValue( MDL_SHIFT_Y, 0.0, objId)
    ...
    for shiftX, shiftY in izip(shiftListX, shiftListY):
        if alFrame >= s0 and alFrame <= sN:
            ...
            globalShiftsMD.setValue( MDL_IMAGE, imgFn, objId)
            globalShiftsMD.setValue( MDL_SHIFT_X, shiftX, objId)
            globalShiftsMD.setValue( MDL_SHIFT_Y, shiftY, objId)
        alFrame += 1
...
def writeMovieMd(movie, outXmd, f1, fN, useAlignment=False):
    ...
    for i in range(f1, fN+1):
        ...
        if useAlignment:
            ...
            row.setValue( MDL_SHIFT_X, shiftListX[shiftIndex])
            row.setValue( MDL_SHIFT_Y, shiftListY[shiftIndex])
