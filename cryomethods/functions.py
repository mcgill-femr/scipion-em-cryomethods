...
def compute_ctf_torch(freqs, dfu, dfv, dfang, volt, cs, w, phase_shift=0, bfactor=None):
    """
    Compute the 2D CTF
    Input:
        freqs (torch.Tensor): Nx2 tensor of 2D spatial frequencies
        dfu (float): DefocusU (Angstrom)
        dfv (float): DefocusV (Angstrom)
        dfang (float): DefocusAngle (degrees)
        volt (float): accelerating voltage (kV)
    ...
    # Compute wavelength lam = sqrt(h^2/(2*m*e*Vr)); Vr = V + (e/(2*m*c^2))*V^2
